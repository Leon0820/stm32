  ...
  if(LCD_ReadReg(0x00) == 0x8989)
  {
    /* Start Initial Sequence ------------------------------------------------*/
      LCD_WriteReg(0x0000,0x0001);       /* 打开晶振 */
	    ...
	    LCD_WriteReg(0x0001,0x2B3F);      /* 驱动输出控制320*240 0x2B3F */
	    LCD_WriteReg(0x0002,0x0600);   
	    LCD_WriteReg(0x0010,0x0000);   
	    LCD_WriteReg(0x0011,0x6070);     /* 定义数据格式 16位色 横屏 0x6070 竖屏 0x6058*/
...
/*******************************************************************************
* Function Name  : LCD_ShowPicture
* Description    : 显示一张图片，直接扫描一页
...
{
  uint32_t index=0;
	uint16_t *Color=(uint16_t *)Pict;
		
	LCD_SetCursor(0,0);     //坐标定位到（0，0） 	
    LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */
